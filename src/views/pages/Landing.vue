...
const fetchProducts = async () => {
    await getProducts({ name: name.value, category: category.value, inventoryStatus: inventoryStatus.value, rating: rating.value }).then((data) => {
        products.value = data.data;
    });
};

const addFavoriteProduct = async (product) => {
    await updateProduct(product.id, { favorite_id: null });
    ...
    await addFavorite(initialPayload)
        .then((response) => {
...
const deleteFavoriteProduct = async (product) => {
...
const addToCart = async (product) => {
    ...
    await addProductCart(payload).then(() => {
        updateProduct(product.id, payload).then(() => {
            fetchCart();
            fetchProducts();
            isCart.value = true;
...
const productCounts = computed(() => {
    ...
    return Object.entries(countMap).map(([product_id, quantity]) => ({
